...
class BuildersGame:
    ...
    def initial_placement(self,x1,y1,x2,y2,player):
        ...
        if player == 'x' and self._turn!='o' and self._xbuilder1 is None and self._xbuilder2 is None: 
            self._turn = 'o'
            self._xbuilder1 = str(x1) + str(y1) 
            self._xbuilder2 = str(x2) + str(y2) 

        elif player == 'o' and self._turn!='x' and self._obuilder1 is None and self._obuilder2 is None:
            self._turn='x'
    ...
    def boundary_check(self,x,y):
       """check if player stays in the board"""
    ...
    def check(self,x,y):
         """check if player makes legal move"""
         
         if boundary_check(self,x-1,y-1)=="True" and self._board[x-1][y-1]-level<=1:
            return True
         elif boundary_check(x-1,y)=="True" and self._board[x-1][y]-level<=1:
            return True
         elif boundary_check(x-1,y+1)=="True" and self._board[x-1][y+1]-level<=1:
            return True
         elif boundary_check(x,y-1)=="True" and self._board[x][y-1]-level<=1:
            return True
         elif boundary_check(x,y+1)=="True" and self._board[x][y+1]-level<=1:
            return True
         elif boundary_check(x+1,y-1)=="True" and self._board[x+1][y-1]-level<=1:
